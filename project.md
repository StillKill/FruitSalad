...
- Итоговый overlay показывает:
  ...
  - breakdown выбранного игрока по каждой салатной карте.
...
## План
- Поддерживать `changes.md` как краткий журнал завершенных задач.
- Провести отдельный spacing/layout pass по всему проекту: вертикальные отступы, плотность строк, выравнивание подпунктов и согласованность text blocks между панелями.
- Улучшить `end_game` overlay: сделать breakdown визуальным списком карт с явным отображением, сколько очков принесла каждая карта, и усилить акценты мест/summary-метрик.
- Языки: RU.
- Таймер хода.
