...
- Для верхней карты выбранной колоды доступна отдельная кнопка переворота в `backFruit`, а для салатов в области активного игрока используется отдельный flip mode с выбором одной карты следующим кликом.
